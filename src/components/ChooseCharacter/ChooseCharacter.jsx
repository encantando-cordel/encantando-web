...
export default class ChooseCharacter extends Component {
    ...
    handleMouseClick(e) {
        ...
        } else if (this.state.imgWoman === WomanSelected && e.target.id === "manCharacter") {
            this.setState({imgMan: ManSelected, imgWoman: Woman});
        } else {
            (e.target.id === 'manCharacter') ? this.setState({imgMan: ManSelected}) : this.setState({imgWoman: WomanSelected});
        }
    }
    ...
    render() {
        return (
            <div className = "main">
                ...
                <img id = "womanCharacter" src = {this.state.imgWoman} 
                    onClick= {this.handleMouseClick}  alt=""
                />
